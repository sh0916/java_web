			...
			for (int i = 0; i < items.size(); i++) {
				...
				// form 요소인지 판별
					...
					if(fileItem.getFieldName().equals("userId")) {
						...
						loginDTO.setUserId(fileItem.getString());
					} else if(fileItem.getFieldName().equals("userPass")) {
						
						loginDTO.setUserPass(fileItem.getString());
					} else if(fileItem.getFieldName().equals("imgName")) {
						
						loginDTO.setImgName(fileItem.getString());
					}
					
					LoginDAO loginDAO = new LoginDAO();
					loginDAO.regi(loginDTO);
				} else {
				// file 영역
